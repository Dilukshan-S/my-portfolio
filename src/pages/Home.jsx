...
export default function Home() {
  ...
  const trackCVDownload = () => {
    console.log("[Analytics] CV Download clicked at", new Date().toISOString());
    // Optional: Replace with Google Analytics or Mixpanel
    // gtag('event', 'cv_download', { timestamp: new Date().toISOString() });
  };
      ...
      >
        ...
        >
          <motion.a
            ...
            className="magic-btn"
          >
            <FaDownload className="text-lg z-10" />
            <span className="z-10">Download CV (PDF)</span>

            <span className="absolute inset-0 bg-black/5 z-0"></span>
          </motion.a>
        </motion.div>
      </div>
